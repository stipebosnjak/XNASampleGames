...
using Frend.V2.Goap.GoalEvaluators;
using Xna.Helpers;
...
{
    public class GoalThink :  GoalComposite<Tamagotchi>
    {
        private readonly List<GoalEvaluator<Tamagotchi>> _goalEvaluators;

        public GoalThink(Tamagotchi character, GoalType type)
            : base(character, type)
        {
            ...
            _goalEvaluators = new List<GoalEvaluator<Tamagotchi>>();
        ...
        {
            float best = 0;
            GoalEvaluator<Tamagotchi> mostDesirable=null;
             
            //iterate through all the evaluators to see which produces the highest score

            foreach (GoalEvaluator<Tamagotchi> goalEvaluator in _goalEvaluators)
            {
